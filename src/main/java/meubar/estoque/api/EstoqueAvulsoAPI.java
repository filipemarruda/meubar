...
import meubar.estoque.servico.ServicoEstoqueAvulso;
...
public class EstoqueAvulsoAPI extends BaseAPIImpl {

	@EJB
	ServicoEstoqueAvulso servico;

	public EstoqueAvulsoAPI() {
	}
	...
	public Response doGet(@CookieParam("auth_token") String token, @PathParam("id") String id) {
		...
		try {
			...
			EstoqueAvulsoJson item = servico.getById(id);
	...
	public Response doPost(@CookieParam("auth_token") String token, String json) {
		...
		try {
			...
			EstoqueAvulsoJson itemJson = gson.fromJson(json, EstoqueAvulsoJson.class);
