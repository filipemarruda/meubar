...
import meubar.estoque.servico.ServicoCategoria;
...
public class CategoriaAPI extends BaseAPIImpl {

	@EJB
	ServicoCategoria servico;
	...
	public Response doGet(@CookieParam("auth_token") String token) {
		List<CategoriaJson> list = servico.getAll();
		Gson gson = new Gson();
	...
	public Response doGet(@CookieParam("auth_token") String token,
		...
		CategoriaJson item = servico.getById(id);
		Gson gson = new Gson();
	...
	public Response doPost(@CookieParam("auth_token") String token, String json) {
		...
		try {
			...
			CategoriaJson itemJson = gson.fromJson(json, CategoriaJson.class);
			itemJson.setUsuarioId(getUsuarioIdFromToken(token));
			result = servico.cadastrar(itemJson);
