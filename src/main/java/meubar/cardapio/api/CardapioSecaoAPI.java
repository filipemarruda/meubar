...
import meubar.cardapio.servico.ServicoCardapioSecao;
...
public class CardapioSecaoAPI extends BaseAPIImpl {

	@EJB
	ServicoCardapioSecao servico;

	public CardapioSecaoAPI() {
	}
	...
	public Response doGet(@CookieParam("auth_token") String token, @PathParam("id") String id) {
		...
		try {
			...
			CardapioSecaoJson item = servico.getById(id);
	...
	public Response doPost(@CookieParam("auth_token") String token, String json) {
		...
		try {
			...
			CardapioSecaoJson itemJson = gson.fromJson(json, CardapioSecaoJson.class);
