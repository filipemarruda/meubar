...
import meubar.cardapio.servico.ServicoCardapioItem;
...
public class CardapioItemAPI extends BaseAPIImpl {

	@EJB
	ServicoCardapioItem servico;

	public CardapioItemAPI() {
	}
	...
	public Response doGet(@CookieParam("auth_token") String token, @PathParam("id") String id) {
		...
		try {
			...
			CardapioItemJson item = servico.getById(id);
	...
	public Response doPost(@CookieParam("auth_token") String token, String json) {
		...
		try {
			...
			CardapioItemJson itemJson = gson.fromJson(json, CardapioItemJson.class);
