...
produtosApp.controller('ProdutoCtrl', ['$filter','$scope', '$cookies', '$stateParams', '$rootScope', '$location', 'Produto', 'Categoria', 'Unidade',
	function($filter, $scope, $cookies, $stateParams, $rootScope , $location, Model, Categoria, Unidade) {

		$scope.moduleConfig = new ModuleConfig(produtosApp.name);
		...
		$scope.remove = function(item) {
			if (item) {
				if(Utils.showConfirmDialog('Deseja realmente escluir o produto "' + item.nome + '"?')){
					var i = new Model({
						id: item.id
					});
