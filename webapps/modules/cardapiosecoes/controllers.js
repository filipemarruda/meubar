...
cardapiosecoesApp.controller('CardapiosecoesCtrl', ['$filter','$scope', '$cookies', '$stateParams', '$rootScope', '$location', 'Cardapiosecoes',
	function($filter, $scope, $cookies, $stateParams, $rootScope , $location, Model) {

		$scope.moduleConfig = new ModuleConfig(cardapiosecoesApp.name);
		$scope.moduleHeader = "Seções de Cardápio";
		...
		$scope.remove = function(item) {
			$scope.clear();
			if (item) {
				Utils.showConfirmDialog('Excluir Seção', 'Deseja realmente escluir a seção do cardápio "' + item.nome + '"?',
					function(){
						var i = new Model({
							id: item.id
						});
